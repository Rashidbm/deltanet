...
export function CloudWorkSurface({ step, playing }: { step: number; playing: boolean }) {
  ...
  return <div className="cloud-desktop" data-step={step} data-playing={playing}>
    ...
    <div className="cloud-browser">
      <div className="cloud-browser-tabs" aria-hidden="true"><div className="mac-window-controls"><i /><i /><i /></div><div className={!brief ? 'is-current' : ''}><img src={assetUrl('/apps/github.svg')} alt="" />Release checks</div><div className={brief ? 'is-current' : ''}><img className="delta-tab-icon" src={assetUrl('/logo-options/split-delta.svg')} alt="" />Morning brief</div></div>
      ...
      <div className="cloud-browser-viewport">
        <motion.div className="cloud-job-view" animate={{opacity:brief ? 0 : 1,transform:brief && !reduced ? 'translateY(-6px)' : 'translateY(0px)'}} transition={{duration:reduced ? 0 : .22}} aria-hidden={brief}>
          <header><div><img src={assetUrl('/apps/github.svg')} alt="GitHub" /><span>checkout <b>/</b> Actions</span></div><span>#482</span></header>
